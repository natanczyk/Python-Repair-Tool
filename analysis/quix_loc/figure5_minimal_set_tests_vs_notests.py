"""
Figure for Table 5 (QuixBugs): Minimal set F1 with test feedback vs. without
test feedback, per LLM. Grouped bar chart. FauxPy (top-|GT|) is drawn as a
reference line, matching Table 3's baseline.
"""
...
from table3_minimal_set_llm import compute_fauxpy_metrics
...
def main():
    ...
    ax.axhline(fauxpy_f1, color="#E07B39", linestyle="--", linewidth=1.5,
               label=f"FauxPy top-|GT| = {fauxpy_f1:.3f}")
    ...
    ax.set_title("Minimal Set Detection: With Tests vs. Without Tests (QuixBugs)")
    ax.legend(fontsize=9)
    ax.grid(axis="y", alpha=0.3)

    plt.tight_layout()
    fig_path = OUT_FIGURES / "figure5_minimal_set_tests_vs_notests.png"
