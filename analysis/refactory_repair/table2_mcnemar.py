"""
Table 2. Paired significance testing (McNemar's exact test) -- does
localization guidance actually improve repair over blind (`none`) repair?
...
def mcnemar(a_solved: pd.Series, b_solved: pd.Series) -> tuple[int, int, float]:
    """Returns (b_only, a_only, two-sided exact p-value) where b_only = A failed & B solved,
    a_only = A solved & B failed."""
...
def build_table2() -> pd.DataFrame:
    ...
    for model in MODELS:
        ...
        # restrict to submissions present under all 4 types (should be all, on comparable basis)
...
if __name__ == "__main__":
    ...
    print("Format: <net change in solved count> (p-value, significance)\n")
